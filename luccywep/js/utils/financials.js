...
function generateAssistantTip(consumptionPercentage, freeBudget) {
    if (consumptionPercentage === 0) {
        return "¡Qué gran comienzo! Tu presupuesto de ahorro libre está intacto en $ " + freeBudget.toFixed(2) + ". Registra tu primer gasto presionando el botón '+ Gasto'.";
    }
...
function generateLuccyReply(cleanMsg, user, availableToSpend, consumptionPercentage, fixedExpenses, totalSpent, income, freeBudget, totalFixed) {
    if (cleanMsg.includes('hola') || cleanMsg.includes('buenos dias') || cleanMsg.includes('buenas tardes')) {
        ...
        return `Comprendo tu inquietud sobre tu dinero. Tu ingreso total actual es de $${income.toFixed(2)}. Tienes disponibles $${availableToSpend.toFixed(2)} de presupuesto libre tras cubrir tus fijos de $${totalFixed.toFixed(2)}. Te recomiendo seguir registrando tus gastos cotidianos para poder darte un análisis más certero.`;
    }
}
